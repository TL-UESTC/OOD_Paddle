    ...
    def training(self, checkpoint = -1):
        ...
        for epoch in range(checkpoint+1, self.epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_loader):                      
                ...
                self.encoder.clear_grad()
                self.decoder.clear_grad()
                self.attr_encoder.clear_grad()
                self.attr_decoder.clear_grad()
                self.classifier.clear_grad()
                ...
                sub_batch_size = 10
                z_x_2 = z1.rsample(sub_batch_size).transpose(1,0,2)
                z_attr_2 = z2.rsample(sub_batch_size).transpose(1,0,2)[input_label]
                ...
                sub_batch_size = 10
                z_x_2 = z3.rsample(sub_batch_size).transpose(1,0,2)
                z_attr_2 = zz.rsample(sub_batch_size).transpose(1,0,2)[self.data.unseen_mask]
                ...
                label_all = paddle.to_tensor(self.data.label_all).astype('float32').flatten(0, -1)
