...
import paddle_torch.optim as optim
...
class Model_train(object):
    ...
    def training(self, checkpoint = -1):
        ...
        # enc_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=self.lr, step_size=10000, gamma=0.5)
        # dec_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=self.lr, step_size=10000, gamma=0.5)
        # attr_enc_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=self.lr, step_size=10000, gamma=0.5)
        # attr_dec_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=self.lr, step_size=10000, gamma=0.5)
        # classifier_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=self.lr, step_size=10000, gamma=0.5)

        # enc_optim = torch.optim.Adam(params=self.encoder.parameters(), lr=enc_scheduler)
        # dec_optim = torch.optim.Adam(params=self.decoder.parameters(), lr=dec_scheduler)
        # attr_enc_optim = torch.optim.Adam(params=self.attr_encoder.parameters(), lr=attr_enc_scheduler)
        # attr_dec_optim = torch.optim.Adam(params=self.attr_decoder.parameters(), lr=attr_dec_scheduler)
        # classifier_optim = torch.optim.Adam(params=self.classifier.parameters(), lr=classifier_scheduler)
        ...
        for epoch in range(checkpoint+1, self.epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_loader):                      
                ...
                if torch.cuda.is_available():
                    ...
                    all_attrs = torch.from_numpy(self.all_attrs).float().cuda()
                ...
                enc_optim.zero_grad()
                dec_optim.zero_grad()
                attr_enc_optim.zero_grad()
                attr_dec_optim.zero_grad()
                ...
                z_attr_2 = z2.rsample(sub_batch_size).permute(1,0,2)[input_label]
                ...
                sub_batch_size = 10
                z_x_2 = z3.rsample(sub_batch_size).permute(1,0,2)
                z_attr_2 = zz.rsample(sub_batch_size).permute(1,0,2)[self.data.unseen_mask]
                ...
                label_all = torch.from_numpy(self.data.label_all).long().view(-1).cuda()
    ...
    def search_thres_by_traindata(self, epoch, dataset = None, n = 0.95):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            ######
            input_data = convertTensor(input_data)
            input_label = convertTensor(input_label).astype('int32')
            input_attr = convertTensor(input_attr)
