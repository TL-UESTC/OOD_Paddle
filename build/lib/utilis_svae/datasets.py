...
import numpy as np
import scipy.io
from paddle_torch.utils.data import Dataset
...
class AwA2(object):
    ...
    def prepare_data(self):
        ...
        self.rawdata = copy.deepcopy(features)
        self.attrs_mat = copy.deepcopy(attr)
        attrs_mat = self.attrs_mat
        labels = features['labels'].squeeze() - 1 # Using "-1" here and for idx to normalize to 0-index
        ...
        self.seen_mask = np.array([(c in seen_classes) for c in range(num_classes)])
        self.unseen_mask = np.array([(c in unseen_classes) for c in range(num_classes)])
        self.label_unseen = np.array(unseen_classes)
        self.label_all = np.array(list(range(num_classes)))
        ##
