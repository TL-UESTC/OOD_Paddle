...
# from paddle_torch.distributions.kl import register_kl
...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    def __init__(self, loc, scale, validate_args=None):
        ...
        # self.loc = loc
        # self.scale = scale
        self.loc = convertTensor(loc)
        self.scale = convertTensor(scale)
    ...
    def rsample(self, shape=torch.Size()):
        ...
        shape = shape if isinstance(shape, list) else torch.Size([shape])
        ...
        return convertTensor(z).astype(self.dtype)
    ...
    def __sample_w_rej(self, shape):
        ...
        s = torch.minimum(torch.maximum(torch.tensor([0.]), self.scale - 10), torch.tensor([1.]))
    ...
    def __while_loop(self, b, a, d, shape):
        ######
        b, a, d = convertTensor(b), convertTensor(a), convertTensor(d)

        # b, a, d = [e.repeat(*shape, *([1] * len(self.scale.shape))) for e in (b, a, d)]
        b, a, d = [e.tile(shape + ([1] * len(self.scale.shape))) for e in (b, a, d)]
        ...
        while bool_mask.sum() != 0:
            ...
            reject = ~ accept
            ######
            accept = convertTensor(accept)
            reject = convertTensor(reject)
        ...
        ######
        # return e, w
        return convertTensor(e), convertTensor(w)
...
@paddle.distribution.register_kl(VonMisesFisher, HypersphericalUniform)
def _kl_vmf_uniform(vmf, hyu):
    return - vmf.entropy() + hyu.entropy()
    
@paddle.distribution.register_kl(VonMisesFisher, VonMisesFisher)
